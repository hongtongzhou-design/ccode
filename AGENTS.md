...
- **终端行为**（用户明确要求）：
  ...
  - **改动面板空信息走本地快速提交**：提交信息非空时原样提交；为空时按文件状态/数量即时生成中性默认信息，直接执行
    ...
    仅在用户主动点 ◈ 时调用 AI。
  - **终端右栏统一称“对话”并采用有界实时视图**：仅展示最近 50 条，标题、agent、会话 ID 与识别/同步/结束状态
    及精确的「完整回放」入口**收进右栏页签行右侧**（走查去重：页签行与对话头部合并为一行，不再单设「工作台」
    标题行与对话区头部；文件/改动页签的上下文由各自内容头部承担）。用户在底部附近时才自动跟随；向上阅读后禁止强制滚动，改显示“有新消息”入口。
  - **终端左栏区块一行式折叠**：「打开的标签」「最近项目」默认收起为一行式区标题（点击展开，组件内记忆；
    打开的标签保留首次有 agent 运行时自动展开一次），项目区 + 文件树为常驻主体；三区标题统一 10px 灰字一行式，
    区间 hairline 分隔，点击区不小于 28px。
...
- **Git 改动列表的单文件 diff 必须安全且可展开**：普通仓库只允许读取当前 status 中经过安全校验的相对路径，工作区只允许读取
  ...
  “当前项目改动”，必须声明它不是历史快照，禁止在对话页提交或推送。紧凑 diff 的增删行**整行铺语义深底**（bg-ok/bg-err，
  七主题共享令牌，v3.36 用户拍板"可以铺"——推翻此前禁铺色约定），hunk 标题用 inset 底。**改动面板为主从分栏布局（v3.36）**：点文件行 = 左栏 diff 主区 +
  右栏紧凑文件列（不再行内展开），右栏保持勾选框与状态徽标；WKWebView 不显示 title 悬浮，操作入口一律用可见提示（hover 才现小字）。
...
- 全站**沉浸冷黑主题**，令牌集中在 `src/App.css` 的 `@theme` + `[data-theme]` 变体（**七套深色**：沉浸黑(默认)/陶土/Ayu琥珀/Catppuccin/极简灰蓝/Dracula/灰蓝正红），运行时 `document.documentElement.dataset.theme` 切换，**改主题只动这一个文件**；不要在组件里散落 hex。**P0 视觉重设计校准（用户拍板）**：默认主题 CTA 换粉 `#faa8d4`（cta-text 近黑）；新增 `--color-raised`（浮起面板/pill 底）、`--color-bubble`（用户消息气泡）、`--color-nav-accent`（侧栏选中左条 + 选中图标着色，默认主题靛蓝、其余主题取各自 CTA 色）。
...
- **线条语言（去格子化，用户反馈"看着像一个个格子区域"后确立）**：内联内容容器（工具栏/分区/面板/列表组/时间线）一律**不加 1px 描边**，靠底色明暗差 + 统一圆角 + hairline 分隔线分层；边框只保留给浮层（弹窗/浮出菜单）与控件（按钮/输入框）。浮起梯度 strip/inset/raised 三级在七套主题下都必须可分辨（v3.35 校准）。**区间分隔优先用留白，hairline 只给真正的内容分界**（v3.37：折叠区标题、rail 底部按钮区不再配横线；错位/交叉的线比线本身更显格子；常规页 PageHeader 也不画底线，标题栏与主体靠留白分层）；hairline/field 与底色的对比度七套主题必须同档（v3.37 前只调柔过默认主题，Dracula 等五套线过硬）。搜索框无描边（inset 底色 + 聚焦加深），表单输入框保留 field 边。**全站线宽 0.5px**（App.css 统一覆写 border/divide 工具类，Retina 真细线、1x 屏兜底 1px；focus outline 不动）。**侧栏线条向 Codex 看齐**：侧栏只保留全高竖分界 + 底部管理区一根横线；品牌区、各页 rail 头部一律不画横线，分区靠留白。**共享控件集中 `PageFrame.tsx`**（primary/secondary/rowAction/ghostAction/field/searchField/hoverReveal 类常量 + SegTabs 组件），禁止各页再复制本地 secBtn/field/hoverReveal；配置页专属别名令牌（pg/grp/pl1/pl2/okb/warnb）已删除，一律用通用语义令牌。编辑器面走 `--color-editor-bg/fg/line` 令牌（七主题各自取值），Monaco 经 MutationObserver 随 data-theme 实时换肤。
...
- **工作台控件尺寸固定分两级**：页面标题栏主/次操作、终端启动栏输入与下拉统一 32px；任务行、步骤胶囊、
  对话列表/回放头部及图标按钮统一 28px。状态标签可以更紧凑，但只要可点击就不得用不足 28px 的点击区；
  主次层级靠填充色、边框和文字色区分，不得再靠同一行按钮忽大忽小表达层级。
- **全站留白节拍固定**：常规页使用统一的标题呼吸区、工具栏间距和主体内边距；不要为了填满窗口扩大卡片或堆叠说明，
  空状态与低对象数量时允许保留连续画布。工作区流水线与任务行可增加垂直间距，但不改变步骤顺序和操作语义。
- **全站导航按工作流分层**：侧栏顺序固定为工作（工作区/终端/对话）→ 能力（配置/技能）→ 管理（统计/设置），
  首次启动默认进入工作区；保留全部页面路由和 visited 保挂载语义。常规页必须复用 `PageFrame/PageHeader/PageToolbar`
  的“上下文标题栏 + 独立筛选工具栏 + 主体”结构，标题栏只保留唯一主动作；终端、对话、技能预览等工作台页面可维持
  自身分栏，但分隔、密度和状态语言必须与共享框架一致。借鉴外部工作台只学对象列表/上下文栏/三栏机制，不照搬其 Git
  客户端信息架构，也不得把配置中心重新设为产品首页。**侧栏按页面自动收展（v3.38，用户否决全局顶栏方案）**：
  终端/对话多列工作页自动收成图标栏，离开后恢复 localStorage 手动偏好；手动折叠/展开置 `navManual`，本 session 停止自动跟随。
- **对话页三栏固定（P1a）**：应用导航 ｜ 会话列表栏（375px，rail2 底：标题+「当前 N · 总计 M」副题 + 深色次按钮 +
  搜索框 + 折叠分类筛选——展开为左右主从栏：左栏 全部/内部/各 agent 一眼看全，
  右栏列出聚焦 agent 的项目——**v3.37 改单列纵向手风琴**（用户反馈左右主从栏"一横一竖难受"）：
  点 agent 只展开/收起项目子列表（左侧缩进线表达层级），「全部项目」/单项目行才落筛选并自动收起），计数保持）｜
  回放区（canvas 底，常驻，未选中为空态）；列表与回放并列常驻，禁止恢复“列表/回放二选一”
  整列切换。会话行双行（标题行带右侧相对时间 + meta 行），选中行 bg-rail-sel 浅填充。`ConversationView` 形态固定：
  用户消息右对齐气泡（bg-bubble，max-w 70%）、AI 直接排版、``` 围栏代码块 inset+hairline 带 ⧉ 复制、连续
  tool_use/tool_result 折叠为「▸ N 次工具调用」行；回放区底部为只读展示态圆角输入条（chip 显示 agent 名）。
  有界尾窗/向前分页保持滚动位置的语义不得改动（scrollRef 必须挂在滚动容器上）。
- **对象数量不得撑坏主布局**：工作区页使用“项目导航 rail + 当前项目详情”，禁止恢复为全部项目纵向长页；技能主列表
  禁止按 Agent 数量永久加列，只展示稳定字段与应用计数，具体 Agent 分发在右侧详情用自动换行网格管理。新增 Agent 时
  只能增加详情项，不得改变技能主表列数或制造横向滚动。终端模块 UI 小字基准为 13px，xterm 新用户默认 14px；状态点等
  微型符号可更小，但文件树、标签、启动栏、对话/改动正文不得退回 11–12px 主阅读字号。
...
- **P1 四条并行线 ✅**：
  ...
  - P1b 流水线骨架 ✅：`.ccode/project.toml` 档案卡 + 项目注册、工作区页按项目分组 + 流水线胶囊概览（状态从工作区派生；进度由等分进度段直接表达，无文字计数，校验提示收进可点击的 ⚠ 浮层）、一键开步（含 bootstrap 自动提交 + TASK.md exclude）、资源面板与自动发现、非 git 目录引导 init；首启引导为轻量版（空流水线走模板选择器），完整版（演示数据 + 示例 PDF）留 backlog；工作区类型驱动默认值（数据类跳端口）未做，留 backlog
