...
export default function WorkspacesPage({ visible }: { visible: boolean }) {
  ...
  const repoCount = new Set(workspaces.map((workspace) => workspace.repoPath))
    .size;
  const groups = [
    ...new Set(workspaces.map((workspace) => workspace.repoPath)),
  ].map((repoPath) => ({
    repoPath,
    repoName: workspaces.find((workspace) => workspace.repoPath === repoPath)!
      .repoName,
    list: workspaces.filter((workspace) => workspace.repoPath === repoPath),
  }));
  const actionBtn =
  ...
  return (
    <div className="h-full overflow-auto">
      ...
      {groups.length === 0 ? (
        ...
        groups.map((group) => (
          <section key={group.repoPath} className="mb-5">
            <h2
              className="mb-1 text-sm font-medium text-l1"
