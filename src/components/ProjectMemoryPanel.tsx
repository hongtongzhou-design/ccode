...
export default function ProjectMemoryPanel({ projectPath }: { projectPath: string }) {
  ...
  async function save(action: "add" | "replace" | "revoke", id: string | null) {
    if (!doc || busy) return;
    if (!reason.trim()) { setError("请写明确认或作废的依据"); return; }
    if (action === "revoke" && !await confirmDialog("作废这条项目知识？历史仍保留，之后开工不再把它当作有效结论。", { focusCancel: true })) return;
  ...
  return <section aria-label="项目长期知识" className="space-y-2 text-xs">
    ...
    {doc?.entries.filter((e) => e.meta.state === "active").map((entry) => <article key={entry.meta.id} className="rounded bg-strip p-2">
      ...
      {!!entry.stalePaths.length && <p className="text-warn-text">来源已变化，当前不作为有效结论：{entry.stalePaths.join("、")}</p>}
    ...
    {!!doc?.legacyText && <details className="text-l3"><summary>旧知识原文（未绑定版本）</summary><pre className="max-h-40 overflow-auto whitespace-pre-wrap">{doc.legacyText}</pre></details>}
