...
- **全站界面按工作面收敛，不换皮（2026-09-09，第一波已落地）**：对照 Linear 与十五张截图评审，病在空间分配和主次，不在配色。不重做导航、不加主题、不把青绿写成产品默认（自定义主题的强调色归用户）。规则：① 项目任务井 container 窄于 56rem 时右侧对话改抽屉，开关 `ccode.projectSessionsOpen`；② 空态只回答下一步（无目标 EmptyState；终端去掉点阵，一个「运行」；标签不用 `~`）。工作台「正在进行」只在有活进程时使用。对话回放跨消息合并纯工具调用为「执行记录」。编程工作树行保留进入/改动/拉取/推送符号，其余进 ⋯。浮层禁用原生 select（`MenuSelect`），目标页去掉套卡，流程线去掉绿字与来源条黑底。第二波（自定义浅色降饱和、文件/Agents）与第三波（资源页精修）未做。

- **会话聊完自动起名（2026-09-09，已落地）**：展示标题格式 `MMDD|类型|主题`；日期用会话创建日、上海时区，不用更新时间或当天。类型闭集：功能/设计/修复/优化/发布/探索/文档/研究。开头只根据第一条真正的用户问题写临时标题；Agent 退出后再用用户原话（首条意图 / 中途纠正 / 最后定题）校正，不送助手回复。写入 `session_meta.custom_title`，`title_source=auto`，不写回 CLI 源文件。人手改过的标题（`title_source=user`）不覆盖；问候-only / 无创建日 / 内部无头会话保持原标题。配置复用「会话摘要」profile。外部 CLI 自行发起的对话不自动起名。未做 Mesa Dev 实机界面验收。
...
- **验收接受记录长期化（2026-09-09，已落地）**：普通目标采纳写文件成功后先写 append-only 账本 `.ccode/acceptance-log.jsonl`（目标/Run/路径/意见/是否冻结/时间，按 run_id 去重，重试幂等）；账本落盘失败 = 采纳返回可见错误引导重试，不静默成功。`project-status.json` 降为账本的「最近摘要」投影，摘要写失败记 logbuf + 事件。删除目标只清工作数据（Run/事件/隔离目录/评审证据），不再连带删接受摘要；账本随项目 `.ccode/` 存续。目标「删除即归档」完整语义与账本界面展示留后续拍板。未做 Mesa Dev 实机界面验收。
...
- **项目级技能与执行版本记录（2026-09-09，审计 §4.9 前半，已落地）**：`SkillDto.contentDigest` = 库目录清单哈希（与漂移检测同一口径，list 现算）；项目档案卡新增 `skills = [...]` 选用名单（空不写行，解析去重去空白、坏值进 warnings）；规则面板新增「项目技能」勾选区。选用技能进上下文包「项目技能」段（名称 + 版本前 8 位 + 读取/产出契约；库里没有如实标「未安装」），并随 §4.8 的 context.json 快照冻结——「本次用的是技能的哪个版本」由此可查。步骤级挂载（有流程科研）与本字段互补不合并。§4.9 后半（定时采纳从文献白名单扩展为技能声明的通用结果契约、科研复现验收脱离 workspace_id）未做。未做 Mesa Dev 实机界面验收。
