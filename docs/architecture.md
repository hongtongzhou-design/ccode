...
| v3.125 | **同会话双界面与分叉聊天**：主工作区在聊天/终端之间切换但共享同一 PTY、xterm、滚动缓冲和会话文件；聊天发送复用 PTY、resume 与自动启动，不建立第二套上下文。右侧工作台移除实时对话，只保留文件/改动。会话关联成功后用专用 watcher 监听 JSONL 或 OpenCode DB/WAL，事件触发最近 50 条消息整段替换，签名轮询仅作兜底并用请求代次防旧结果覆盖。新建聊天通过脱敏、64KB 限制的结构化 handoff 简报分叉，继承 Agent/连接/模型/目录，默认独立 PTY + 只读；允许修改需确认风险并重启可写 PTY。实时同步定义为消息级准实时，不承诺 token 级解析；分叉不复制完整历史。 |
