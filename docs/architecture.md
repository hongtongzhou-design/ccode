...
- **网关与绑定入口一致（2026-09-08）**：设为全局对 Mesa 管理字段做完整替换（模型槽变短、思考档清空、空名单都要删旧值）。绑定更新只改绑定字段，不回写共享网关 URL/Header/密钥；跨窗口保存用内容指纹拒绝覆盖。导入按绑定身份规则还原多条模型配置，不按 Agent+网关静默合并。体检按槽+模型+地址/密钥指纹消费，思考档与采样分开发、分字段记录；迟到回包对不上当前指纹则丢弃。获取目录合并策略不覆盖未保存值；脏端点/密钥须先保存再测/拉目录。未做九家联网验收与 Mesa Dev 窗口点选。
