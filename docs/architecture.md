...
> 一个「AI 科研工作台」：以 Project 为家，让不同 Agent 在长期环境里完成目标、结果经人验收。底层仍是九个 Agent CLI 的适配（启动器 + 配置 + 会话只读），表面是科研流水线，并覆盖编程/办公。**AI 负责干活，Ccode 负责环境和验收，人负责拍板。** 不是 CLI 聚合器，也不是任务编排器。
