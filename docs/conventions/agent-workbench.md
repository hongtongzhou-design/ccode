...
Ccode 是 **Agent 的项目工作环境**：以 Project 为家，给人文件、规则、上下文；人只说要完成什么；Agent 自己规划；改文件必须验收才进项目。CLI 是第一种 Runtime，不是产品本身。用户名词是项目 / 文件 / 目标 / 验收。Run 是账本，不进手册正文。
...
- 页签按项目路径记忆。
- 「新建目标」出现在工作项目和无流程科研：写一句话，默认整仓进隔离副本，验收后写回。＋新对话默认直接改项目根，可选「验收后写入」。不自动建分类空目录。有流程科研和编程不提供这条入口。禁止自动拆任务与智能路由。
