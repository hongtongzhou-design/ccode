...
- **开工弹层「未登记文件」不重复点名已接到的输入**（2026-09-18）：`discover_resources` 扫到的
  `papers/*.pdf` 若已是本步骤 `inputs` / `optionalInputs` / `anyOfInputs`（精读步即 `papers/*.pdf`），
  上面「上一步接到」已经计数，不再列入「发现 N 个未登记文件」。检索步 Agent 按技能把开放获取 PDF
  直写项目根 `papers/`（`作者年份-短标题.pdf`），不走 `register_pdf`，本来就不会进档案卡资源名单；
  付费墙「获取全文 / 关联本地 PDF」才会同时落盘+登记。不登记也能开工。资源面板「发现资源」仍可补登。
