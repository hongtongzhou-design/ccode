...
- 保留六套模板/原阶段数；工具是项目选择，不增设独立编排系统，不自动路由或自动并行。选择保存在 settings 的闭集 `科研工具/<key>：<value>`，由 `research-tools.ts` 在应用/编辑步骤时补全技能、必需子集、交付、人工事项。来源与外部库交付分开；只认一份主 references.bib。
