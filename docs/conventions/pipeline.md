...
- `decisions` 的非空问题、空 `options` **必须跨编辑器/TOML 保存保留**：表示需人填写版本/证据依据，不是无效条目。流程线上显示「填写依据…」，不参加「全部用推荐值」；全为此类条目时不显示批量推荐。硬暂停检查的是决定状态，不是「有没有非空文字」；不声称验证签字人、依据真实性或阻止 CLI 自身越权。批准与准备绑定当时读到的报告指纹，指纹变化后需重新确认。
