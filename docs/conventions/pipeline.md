...
- **评审壳按步骤类型拼区块**（闭集 `screening` / `files` / `acceptance` / `default`，`step-review.ts`）：一张 `WorkspaceReviewView`，档案声明证据主面、验收表单、文件分组与默认 diff。禁止每步一张独立评审页。检索步走 screening（清单/过程/文件）。精读、大纲、写作、排版、投稿材料走 files（笔记/稿件/引文分组，无科研决定表）。实验执行、结果分析、清洗、EDA 才走 acceptance（验收摘要/复现/接受退回）。有步骤但不是前两类时默认 files，不再把所有非检索步塞进实验验收壳。
...
- **两个告知触点，全部只提醒不阻断**：① 开工确认弹层「人工事项」区（步骤声明了才渲染；before 档未完成时
  ...
  **活着的 Agent 进程优先于 git ahead**（2026-09-18）：中途提交不能把步骤打成「去评审 / Agent 已跑完」）；
