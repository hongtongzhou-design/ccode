...
- **合并带回（gitignored 派生产物）**：git merge 成功后、归档前，把工作区未跟踪/被忽略的 `papers/**`、项目产物目录、`output/**` 拷到主仓同相对路径（同名冲突点名交人、保护路径跳过、失败不阻断合并）。口径 C（2026-09-09）起这是派生产物进主仓的**正常通道**：Agent 派生产物写工作区，评审合并时才进主仓；直写项目根的派生产物是未验收产物。
