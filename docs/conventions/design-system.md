...
- **项目详情页固定三段（v3.85）**：①**身份段**（项目名 16px semibold + 课题主题；主题为空时给可点的
  ...
  **工作方式分流（v3.181 / v3.183）**：无流程科研（`pipelineOptOut`）主区改文献 / 笔记 / 数据 / 雷达 / 定时任务（有内容默认折叠，空则一行式；PDF 连续滚动就地预览），不渲染科研工作区列表；
  ...
  编程主区是左工作树右会话+定时：主按钮「从 <基准> 开工」，进入 / 查看改动 / 拉取·推送组合 / 其余进 ⋯；git 事实用芯片（待合入 / 落后基准 vs 未推送 / 远程有更新，HoverTip 说坐标，禁止 WKWebView title）。会话是可收起侧栏（全高竖分界、零阴影），标题本地清洗不调用 AI；办公 / 无流程科研 / 编程侧栏都有「＋ 新对话」。github.com 已推送的树上常驻「打开 Pull Request」；Desktop 用 AppWindow 打开**这一目录**。冲突排第一行给「去解决」，其余行「查看改动」直达该工作树改动面板。办公主区是文档库（左文档、右对话+定时），文件少于 4 个时不单独出「继续上次」卡、在列表标最近编辑；文件多时「继续上次」与文档井同底，不另垫 raised 白板。文件行「进行中」只认对着该文件的活标签。文档区可搜文件名或路径，子路径只在有子目录时显示。
  **项目页内容井**（文档 / 笔记 / 文献 / 雷达 / 工作树 / 定时任务卡）用 `.ccode-well`：`color-mix(canvas 40%, strip)`，落在 canvas→strip 之间（strip 略占优、画布托色相）；浅色不漂成白纸，也不贴死画布。禁对这些井再用 `bg-strip`/`bg-raised`。
  ...
  雷达与本项目会话跟文献同一套：FoldMark 标题、strip 列表、先 10 条，底部「显示全部 / 收起」。
...
- **会话整理与长回放口径统一**：对话页默认从普通项目树排除 `internal=true`，以及 Ccode 自建的 `ccode-ai-<uuid>` 临时 cwd（无头调用用完即删，登记过期后会话文件还在），归并为单一“Ccode 内部 AI”入口；“显示已归档”
