...
- **项目 Agents 两列名册（2026-09-11，用户定稿）**：保留 `grid-cols-1 md:grid-cols-2` 的原有响应式列数，不改为单列短卡表；卡片用 `.ccode-well`，项目默认用 `bg-seg-sel` 轻高亮，不叠描边/左色条。Agent 与常驻默认操作同一行，连接名/模型分行且长内容截断；品牌圆点只标身份，不代表运行状态。页头说明只留一行，入口叫「管理连接」；编程无工作时不再重复开工提示。配置保存、任务归属和默认选择的解析规则保持不变。
