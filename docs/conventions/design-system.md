...
- 配置页结构（用户详版规格，v3.93 修订）：可折叠 agent 分组（**卡片外框**：field 细边 + strip 底色、未安装组去底色降权——
  ...
  顶部筛选与搜索；官方账号状态收纳为「● 官方账号 · 已连接/未连接」微型标签（排查指引进悬浮 tooltip，连接=次级胶囊、
  刷新=⟳ 图标钮）；配置冲突提升为组头琥珀胶囊（bg-warn，点击弹层列明细）；模型列 Code Tag（前 3 个 + +N 折叠，title 全文）；
...
- **列表行内操作分两级显隐**：每行最多一个常驻主按钮（编辑 / 应用开关 / 评审 / 组头「新版」），其余低频按钮（行内 ⋯、⧉ 复制
  ...
  **行内多操作钮的容器按页面密度分档**（v3.93）：表格化网格行（技能页）可套「raised 底 + hairline 边 + 投影」的
  悬浮胶囊栏；素列表行（MCP 页）只许裸图标钮 hover 淡入——实体胶囊栏压在轻量列表上会显得层级脱节（用户实测否决）。
