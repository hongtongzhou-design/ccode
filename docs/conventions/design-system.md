...
- **项目详情页固定三段（v3.85）**：①**身份段**（项目名 16px semibold + 课题主题；主题为空时给可点的
  ...
  **项目页内容井**（文档 / 笔记 / 文献 / 雷达 / 工作树 / 定时任务卡）用 `.ccode-well`：深色走 `inset`（卡片档，否则混画布后工作树卡看不见层）；浅色仍 `color-mix(canvas 40%, strip)`，不漂成白纸、也不贴死画布。禁对这些井再用 `bg-strip`/`bg-raised`。
  ...
  **空文献雷达**不是第四块等权卡片：一行「文献雷达 · 还没订阅读」+「添加」；有订阅才出完整雷达卡。
