...
- **Run 与标签（v3.221 / v3.223）**：终端标签是一次 Run 的视图，不是身份。同一 `reuseKey` 只留一个活标签；标题先任务名（工作区/分支/文档）后 Agent（Agent 只在进程起来后的状态栏）。交互 `pty_spawn` 必先登记 `runs` 行（登录标签除外）；无头标 `internal` 不进工作台。重启白名单含 `runId`，仍禁 PTY/密钥/env。细则 `docs/conventions/agent-workbench.md`。
