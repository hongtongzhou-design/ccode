...
- **顶部终端标签（v3.135 / v3.153）**：终端标签名与「＋」放进同一个 Ghostty 式轻量圆角胶囊；显示层、成果面板和布局入口不并入标签胶囊。聊天/终端是同一位置的单击切换（图标显示当前层，再点切到另一层）；成果面板是标签栏右侧常驻按钮（与左侧文件树按钮对仗）；布局菜单只保留分屏与专注终端。专注操作菜单只在 `focusMode=true` 时显示「退出专注终端」，快速开聊退出专注后不得残留该菜单项。
- **终端菜单分工（v3.136 / v3.153）**：启动栏 `⋯` 只保留当前标签的启动/会话动作（打开 Shell、高级启动选项、转为项目）；顶部标签栏 `⋯` 只保留跨标签操作（查找、修改启动配置、接力/恢复等）。布局菜单只保留分屏和专注终端，不列出聊天/终端显示层，也不重复列出文件树/成果面板（二者已有独立按钮）；查找不在两个菜单重复出现。
...
- **Excel 宽表预览（v3.164）**：`.xlsx`/`.xls`/`.ods` 只读网格（列字母 + 行号冻结、横滑看全列；点格子顶栏显示全文）。后端 `read_sheet_preview` 最多 200 行 × 256 列并回 `totalRows`/`totalCols`；多工作表可切换。复杂公式/图/宏仍以 Excel 为准。
...
- **文件树折叠钮与双击进入分离（v3.157）**：折叠 chevron 只负责展开/收起（连点不进入）；行上其它区域单击仍展开/收起，
  双击目录才重定根。原先整行同时绑单击折叠和双击进入，点折叠钮两次会被当成双击打开该文件夹。
...
- **文件树默认可收起（用户拍板）**：首次进入不展开文件树；收起后不留窄条（与项目页项目列表同款），
  标签栏左侧「显示文件树」打开，显式开关写入 `ccode.terminal.treeOpen`。布局菜单不再重复此开关。
  专注终端 / 快速开聊 `pt.clean` 仍走既有专注终端，不另造显隐状态。
- **成果面板常驻按钮（v3.153）**：标签栏右侧「显示成果面板」与左侧文件树按钮对仗；点按切换并写入
  `ccode.terminal.rightOpen`。打开后同一按钮变为「隐藏成果面板」（选中态）。专注终端下隐藏该按钮
  （面板本身也被专注态收起）。页内点文件或交接请求仍可临时打开右栏。
...
- **聊天层窥视终端（v3.168）**：聊天与终端仍是同一 PTY 的两层，不是两个会话。窥视 = 底栏真实分栏（约 32% 高度），终端按此高度 fit，TUI 才能画进可见区域。不能只裁全高 xterm 的底部——Codex 主对话是 inline viewport，内容靠上、底下是空行，裁底就是白板。切聊天/终端仍铺满同一容器、行列数不变；**只有窥视开合**会 SIGWINCH 一次（Codex 可能重放历史，这是能看见 TUI 的代价）。默认关；`attention === confirm`、8 秒无会话文件、斜杠 `/model` 无参 / `/models` / `/login`、以及聊天层点 picker 模型时自动打开。用户手动收起后本轮不再自动打开。布局菜单在聊天层提供同一开关。不复制九家 TUI 菜单解析器；交接表在 `src/chat-handoff.ts`（chat_ok / peek / must_switch）。
