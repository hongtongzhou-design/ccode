...
- **标签「生成中」虚线圆（2026-09-13）**：只表示这一轮正在出字，不是进程还活着。用户提交（聊天发送 / 回车）后才允许点亮；**启动或恢复会话不转圈**（TUI 启动/回放画面不是生成回答；只有新会话带了首条指令才点亮）。PTY 有输出才续命。会话窗口最后一条已是助手正文（无待执行工具）或尾部 `done`/`confirm` 立刻停转——不等 PTY 静默、也不让会话文件 sticky `working` 续命。回合结束后的 TUI 残帧不得从 `done` 再点亮。样子是 12px、`text-l3` 的虚线圆沿圆周爬，不上强调色。动画走 `stroke-dashoffset`（圆几何不动），禁止 CSS `rotate`（WKWebView 旋转原点跟着描边外接框走，圆心会晃）。纯逻辑 `src/tab-working.ts`。
